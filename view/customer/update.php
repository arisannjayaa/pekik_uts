...
<body>
    <div class="container">
        ...
        <hr>
        <form action="<?=base_url("/config/customer/update.php")?>" method="post">
            <?php
                $id    = $_GET['id']; 
                $query = mysqli_query($conn, "SELECT * FROM tb_customer WHERE Id_customer = '$id'");
                while ($data = mysqli_fetch_assoc($query)) { ?>
            <div class="row mb-3 visually-hidden">
                <label class="col-2">Id Motor</label>
                <div class="col-4">
                    <input type="text" class="form-control" name="id_customer" value="<?=$data['Id_customer']?>">
                </div>
            </div>
            <div class="row mb-3">
                <label class="col-2">Nama Customer</label>
                <div class="col-4">
                    <input type="text" class="form-control" name="nama" value="<?=$data['Nama_customer']?>">
                </div>
            </div>
            <div class="row mb-3">
            ...
            <div class="row mb-3">
                <div class="col">
                    <a href="<?=base_url("view/customer/index.php")?>"><button type="button"
